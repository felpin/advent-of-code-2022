...
export default class BestSignalFinder {
  ...
  constructor(heightmap: number[][], start: Position, destination: Position) {
    this.heightmap = heightmap;
    this.start = start;
    this.destination = destination;

    this.pointsToVisit = new Queue<PositionVisited>();
    this.pointsToVisit.enqueue({ ...start, steps: 0, history: [] });
  }
  ...
  private visitNextPoint() {
    ...
    if (point.x === this.destination.x && point.y === this.destination.y) {
